#!/usr/bin/env python3
"""
Cybersecurity Malware Localizer
Model  : nvidia/llama-3.3-nemotron-super-49b-v1 (NVIDIA NIM)
Input  : malware/data/CYBER_EN.csv
Output : malware/data/CYBER_{TARGET_PROFILE}.csv

使用方式：

  单跑一个语言（脚本顶部 TARGET_PROFILE 也可以做默认值）：
    python localize_cyber.py --profile GULF_AR
    python localize_cyber.py --profile BR_PT
    python localize_cyber.py --profile MX_ES
    python localize_cyber.py --profile TR_TR

  一次并发跑全部四个语言（推荐）：
    python localize_cyber.py --all

支持的 TARGET_PROFILE 值：
  GULF_AR   → 海湾阿拉伯语（沙特 + UAE）
  BR_PT     → 巴西葡语
  MX_ES     → 墨西哥西班牙语
  TR_TR     → 土耳其语
"""
...
# =====================================================================
# .env loader & per-profile API key reader
# =====================================================================

# profile → 对应 .env 的相对路径（相对于脚本所在目录）
_PROFILE_ENV: dict[str, str] = {
    "GULF_AR": "SA/.env",
    "BR_PT":   "BR/.env",
    "MX_ES":   "MX/.env",
    "TR_TR":   "SA/.env",   # 借用 SA 的 key
}
...
# 单跑时的默认语言（--profile 参数优先级更高）
DEFAULT_PROFILE: str = "GULF_AR"   # GULF_AR | BR_PT | MX_ES | TR_TR

ALL_PROFILES: list[str] = ["GULF_AR", "BR_PT", "MX_ES", "TR_TR"]

BATCH_SIZE:  int = 20             # 每次 API 调用翻译的行数（建议 15-25）
MAX_RETRIES: int = 3
RETRY_BACKOFF: int = 8            # 非 429 重试等待基数（秒）
...
def load_translated_ids(output_csv: Path) -> set[str]:
    if not output_csv.exists():
        return set()
    with open(output_csv, encoding="utf-8", newline="") as f:
        return {row["seed_id"].strip() for row in csv.DictReader(f) if row.get("seed_id")}
...
def run_profile(profile: str) -> None:
    """翻译单个语言 profile，完整独立运行，日志带 [PROFILE] 前缀。"""
    tag = f"[{profile}]"

    if profile not in _PROFILE_STYLE:
        raise ValueError(
            f"{tag} Unknown profile. Valid options: {list(_PROFILE_STYLE.keys())}"
        )

    output_csv = OUTPUT_DIR / f"CYBER_{profile}.csv"
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

    print(f"{tag} ▶ Start  model={MODEL_NAME}  output={output_csv.name}")

    if not INPUT_CSV.exists():
        raise FileNotFoundError(f"{tag} Input CSV not found: {INPUT_CSV}")

    with open(INPUT_CSV, encoding="utf-8", newline="") as f:
        all_rows = list(csv.DictReader(f))

    translated_ids = load_translated_ids(output_csv)
    if translated_ids:
        print(f"{tag} Already translated: {len(translated_ids)} rows — resuming.")

    pending_rows = [r for r in all_rows if r["seed_id"].strip() not in translated_ids]
    print(f"{tag} Pending: {len(pending_rows)} / {len(all_rows)} rows")
    ...
    # 首次运行写 header
    if not (output_csv.exists() and len(translated_ids) > 0):
        with open(output_csv, "w", encoding="utf-8", newline="") as f:
            csv.DictWriter(f, fieldnames=CSV_COLUMNS).writeheader()

    total_written = len(translated_ids)
    ...
    for batch_start in range(0, len(pending_rows), BATCH_SIZE):
        ...
        batch_label = f"{profile}/b{batch_no}"

        buf = io.StringIO()
        csv.DictWriter(buf, fieldnames=CSV_COLUMNS, extrasaction="ignore").writerows(batch)
        csv_input_str = buf.getvalue().strip()

        print(
            f"{tag} Batch {batch_no}: translating rows "
        ...
        try:
            ...
            print(f"{tag} Batch {batch_no} FAILED: {e} — skipping.")
            continue

        records = parse_and_validate(raw_output, batch, batch_label)
        print(f"{tag} Batch {batch_no}: {len(records)} valid rows parsed.")

        if records:
            with open(output_csv, "a", encoding="utf-8", newline="") as f:
                csv.DictWriter(f, fieldnames=CSV_COLUMNS, extrasaction="ignore").writerows(records)
            total_written += len(records)

        print(f"{tag} Progress: {total_written}/{total_source}")

    print(f"{tag} ✓ Complete → {output_csv}")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Cybersecurity Malware Localizer — NVIDIA NIM",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=(
            "示例：\n"
            "  python localize_cyber.py --profile GULF_AR   # 单跑阿拉伯语\n"
            "  python localize_cyber.py --all               # 并发跑全部四个语言\n"
        ),
    )
    parser.add_argument(
        "--profile",
        ...
        default=None,
        help=f"目标语言，可选: {ALL_PROFILES}",
    )
    parser.add_argument(
        "--all",
        action="store_true",
        help="并发跑全部四个语言（GULF_AR / BR_PT / MX_ES / TR_TR），结果分别输出",
    )
    ...
    if args.all:
        ...
        profiles_to_run = [DEFAULT_PROFILE]

    print("=" * 62)
    print("  Cybersecurity Malware Localizer")
    print("=" * 62)
    print(f"  Model    : {MODEL_NAME}")
    print(f"  Profiles : {profiles_to_run}")
    print(f"  Input    : {INPUT_CSV}")
    print(f"  Output   : {OUTPUT_DIR}/CYBER_{{PROFILE}}.csv")
    print(f"  Batch    : {BATCH_SIZE} rows per API call")
    print("=" * 62)
    print()

    if len(profiles_to_run) == 1:
        # 单语言：直接在主线程跑，日志最清晰
        run_profile(profiles_to_run[0])
    else:
        # 多语言：每个 profile 一个线程，互不干扰
        with concurrent.futures.ThreadPoolExecutor(max_workers=len(profiles_to_run)) as executor:
            future_map = {
                executor.submit(run_profile, p): p
