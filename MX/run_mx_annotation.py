#!/usr/bin/env python3
"""
Batch annotation for Mexico data using Nvidia NIM jury.
...
Usage:
  python run_mx_annotation.py                    # default: 20 posts, 3 models
  python run_mx_annotation.py --limit 500        # annotate 500 posts
  python run_mx_annotation.py --limit 1000 --resume  # resume from checkpoint
  python run_mx_annotation.py --dry-run           # show what would be annotated
"""
...
DEFAULT_INPUT = PROJECT_ROOT / "train_MX.csv"
PROMPT_PATH = PROJECT_ROOT / "mx_jury_prompt.txt"
RESULTS_DIR = DATA_DIR / "annotations_mx"
...
JURY_MODELS = {
    "A": {
        ...
        "role": "Spanish/Mexico Language Specialist",
    ...
    "C": {
        ...
        "role": "Senior Mexico Safety Arbitrator",
...
ALTERNATIVE_JURY = {
    "fast": {
        "A": {"name": "google/gemma-3n-e4b-it", "role": "Spanish/Mexico Language Specialist", "timeout": 30},
        "B": {"name": "meta/llama-3.1-8b-instruct", "role": "Cross-Cultural Reasoner", "timeout": 30},
        "C": {"name": "google/gemma-3n-e2b-it", "role": "Senior Mexico Safety Arbitrator", "timeout": 30},
    },
    "multilingual": {
        "A": {
            "name": "google/gemma-3n-e4b-it",
    ...
    "balanced": {
        "A": {"name": "meta/llama-3.1-8b-instruct", "role": "Spanish/Mexico Language Specialist", "timeout": 45},
        "B": {"name": "qwen/qwen3-next-80b-a3b-instruct", "role": "Cross-Cultural Reasoner", "timeout": 45},
        "C": {"name": "mistralai/mistral-nemotron", "role": "Senior Mexico Safety Arbitrator", "timeout": 45},
    },
    "thorough": {
        "A": {"name": "google/gemma-4-31b-it", "role": "Spanish/Mexico Language Specialist", "timeout": 90},
        "B": {"name": "qwen/qwen3-coder-480b-a35b-instruct", "role": "Cross-Cultural Reasoner", "timeout": 90},
        "C": {"name": "mistralai/mistral-large-3-675b-instruct-2512", "role": "Senior Mexico Safety Arbitrator", "timeout": 90},
    },
}

# ── Vote resolution ──────────────────────────────────────────────

# MX-specific violation categories with priority (higher = more severe)
MX_CATEGORY_PRIORITY = {
    ...
    "MX_Narco_Culture_And_Cartel_Glorification": 40,
    "none": 0,
}
...
def resolve_majority_category(categories: list[str]) -> str:
    ...
    best_prio = -1
    for c in categories:
        prio = MX_CATEGORY_PRIORITY.get(c, 0)
        if prio > best_prio:
            best_prio = prio
            best = c
    return best
    ...
    for attempt in range(3):
        try:
            ...
            return {
                ...
                "confirmed_label": data.get("confirmed_label", data.get("category", "none")),
...
def load_mx_data(
    ...
    """Load MX annotation candidates from CSV or parquet."""
    ...
    print(f"Loaded MX data: {len(df):,} rows from {input_path}")
    ...
    if "content_id" not in df.columns:
        df["content_id"] = [f"mx_train_{i:08d}" for i in range(len(df))]
    ...
    for col in ("title", "subreddit", "clean_text", "body"):
...
def build_mx_jury_prompts(
    ...
    """Build three juror prompts from the shared MX policy prompt."""
    policy = load_annotation_policy()
    prompts = {}
    for key in ["A", "B", "C"]:
        cfg = jury_config[key]
        system = f"""You are Juror {key}: {cfg.get("role", "Mexico content safety annotator")}.
...
def run_batch_annotation(
    ...
    for idx, (_, row) in enumerate(to_process.iterrows()):
        ...
        _t = row.get("title")
        _c = row.get("clean_text")
        title_preview = (
            str(_t) if (_t is not None and _t == _t and str(_t).strip())
            else str(_c) if (_c is not None and _c == _c) else ""
        )[:80]
...
def main():
    parser = argparse.ArgumentParser(
        description="Annotate Mexico data with Nvidia NIM jury",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python run_mx_annotation.py                          # full train_MX.csv to mx_annotation_full.csv
  python run_mx_annotation.py --limit 500              # 500 posts
  python run_mx_annotation.py --resume                 # resume from mx_annotation_full.csv
  python run_mx_annotation.py --jury-config fast       # use fast model config
  python run_mx_annotation.py --input path/to/file.csv # use a custom CSV/parquet input
  python run_mx_annotation.py --dry-run                # preview data only
  python run_mx_annotation.py --languages es,en        # Spanish + English
        """,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--languages", type=str, default="es,en",
        help="Comma-separated language codes to include (default: es,en)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Custom output file name (saved in data/annotations_mx/)"
    )
    parser.add_argument(
    ...
    if args.resume:
        if not already_done:
            if custom_output:
                print(f"No existing checkpoint at {checkpoint_path}. Starting fresh.")
            else:
                existing_checkpoints = sorted(
                    list(RESULTS_DIR.glob("mx_annotation_*.parquet"))
                    + list(RESULTS_DIR.glob("mx_annotation_*.csv")),
