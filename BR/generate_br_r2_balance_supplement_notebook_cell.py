...
REFERENCE_FINAL_CSV = BASE_DIR + "/br_synthetic_R2_valid_dedup.csv"
OUT_CSV = BASE_DIR + "/br_r2_balance_supplement_raw.csv"
ERROR_CSV = BASE_DIR + "/br_r2_balance_supplement_errors.csv"
FINAL_CSV = BASE_DIR + "/br_r2_balance_supplement_valid_dedup.csv"
...
MISSING_COUNTS = {
    "Politically_Sensitive_Topics": 9,
    "Sexually_Explicit_Information": 10,
    "BR_State_Security_Democratic_Order": 180,
}

# 补量目标不是最终总目标，而是本次 supplement 文件中希望保留下来的有效去重数量。
# 小缺口给到 30 条，BR 民主秩序缺口大，给 240 条作为余量。
TARGETS = {
    "Politically_Sensitive_Topics": 30,
    "Sexually_Explicit_Information": 30,
    "BR_State_Security_Democratic_Order": 240,
}
...
def load_reference_text_keys():
    """Load existing R2 final texts so supplement rows do not duplicate old data."""
    p = Path(REFERENCE_FINAL_CSV)
    if not p.exists():
        return set()
    try:
    ...
    if "text" not in df.columns:
        return set()
    return set(df["text"].map(normalize_text).tolist())
...
def build_final_dedup(raw_df):
    ...
    for _, r in raw_df.iterrows():
        ...
        key = normalize_text(raw_row["clean_text"])
...
def print_stats(raw_df, err_df, final_df, duplicate_count):
    ...
    for label, target in TARGETS.items():
        ...
        missing = MISSING_COUNTS.get(label, "")
        print(f"{label}: supplement_valid={current} / {target}  original_gap={missing}")
    ...
    print(f"REFERENCE_FINAL_CSV: {REFERENCE_FINAL_CSV}")
