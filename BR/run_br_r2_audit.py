#!/usr/bin/env python3
"""
3-juror audit for BR R2 LLM-generated supplement rows.

This script reuses the same NIM jury models and API rules as run_br_annotation.py,
but loads BR/br_aduit_prompt.txt and audits current_label quality instead of
...
DEFAULT_INPUT = PROJECT_ROOT / "BR_R2_data" / "br_annotation_R2_added_to_balance2000.csv"
DEFAULT_AUDIT_PROMPT = PROJECT_ROOT / "br_aduit_prompt.txt"
DEFAULT_POLICY_PROMPT = PROJECT_ROOT / "br_jury_prompt.txt"
DEFAULT_OUTPUT = PROJECT_ROOT / "quality_audit" / "br_r2_generated_audit.csv"
...
def load_audit_data(input_path: Path, limit: int = 0, labels: list[str] | None = None) -> pd.DataFrame:
    ...
    if "content_id" not in df.columns:
        df["content_id"] = [f"br_r2_audit_{i:08d}" for i in range(len(df))]
    else:
        empty_content_id = df["content_id"].astype(str).str.strip().eq("")
        if empty_content_id.any():
            generated_ids = [f"br_r2_audit_{i:08d}" for i in range(len(df))]
...
{content}"""
    ...
    for key in ["A", "B", "C"]:
        cfg = jury_config[key]
        system = f"""You are Juror {key}: {cfg.get("role", "Brazil generated-data audit juror")}.
...
def resolve_audit_action(current_label: str, vote_result: dict, verdicts: list[dict]) -> tuple[bool, bool, str, str, str]:
    ...
    if not usable or action == "reject":
        return True, False, risk or "medium", "review", "juror_reject_despite_label_match"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Audit BR R2 generated supplement with the 3-juror NIM setup.")
    ...
    policy = load_text(args.policy_prompt.expanduser(), "BR jury policy")
    audit_template = load_text(args.audit_prompt.expanduser(), "BR audit prompt")
    audit_policy = audit_template.replace("{{BR_POLICY}}", policy)
