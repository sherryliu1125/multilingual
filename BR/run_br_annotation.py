#!/usr/bin/env python3
"""
Batch annotation for Brazil data using Nvidia NIM jury.
...
Usage:
  python run_br_annotation.py                    # default: 20 posts, 3 models
  python run_br_annotation.py --limit 500        # annotate 500 posts
  python run_br_annotation.py --limit 1000 --resume  # resume from checkpoint
  python run_br_annotation.py --dry-run           # show what would be annotated
"""
...
DEFAULT_INPUT = PROJECT_ROOT / "train_BR.csv"
PROMPT_PATH = PROJECT_ROOT / "br_jury_prompt.txt"
RESULTS_DIR = DATA_DIR / "annotations_br"
...
JURY_MODELS = {
    "A": {
        ...
        "role": "Portuguese/Brazil Language Specialist",
    ...
    "C": {
        ...
        "role": "Senior Brazil Safety Arbitrator",
...
ALTERNATIVE_JURY = {
    "fast": {
        "A": {"name": "google/gemma-3n-e4b-it", "role": "Portuguese/Brazil Language Specialist", "timeout": 30},
        "B": {"name": "meta/llama-3.1-8b-instruct", "role": "Cross-Cultural Reasoner", "timeout": 30},
        "C": {"name": "google/gemma-3n-e2b-it", "role": "Senior Brazil Safety Arbitrator", "timeout": 30},
    },
    "multilingual": {
        "A": {
            "name": "google/gemma-3n-e4b-it",
    ...
    "balanced": {
        "A": {"name": "meta/llama-3.1-8b-instruct", "role": "Portuguese/Brazil Language Specialist", "timeout": 45},
        "B": {"name": "qwen/qwen3-next-80b-a3b-instruct", "role": "Cross-Cultural Reasoner", "timeout": 45},
        "C": {"name": "mistralai/mistral-nemotron", "role": "Senior Brazil Safety Arbitrator", "timeout": 45},
    },
    "thorough": {
        "A": {"name": "google/gemma-4-31b-it", "role": "Portuguese/Brazil Language Specialist", "timeout": 90},
        "B": {"name": "qwen/qwen3-coder-480b-a35b-instruct", "role": "Cross-Cultural Reasoner", "timeout": 90},
        "C": {"name": "mistralai/mistral-large-3-675b-instruct-2512", "role": "Senior Brazil Safety Arbitrator", "timeout": 90},
    },
}

# ── Vote resolution ──────────────────────────────────────────────

# BR-specific violation categories with priority (higher = more severe)
BR_CATEGORY_PRIORITY = {
    ...
    "BR_State_Security_Democratic_Order": 65,
    "safe": 0,
}
ALLOWED_CATEGORIES = set(BR_CATEGORY_PRIORITY)


def normalize_category(category: object) -> str | None:
    """Normalize common safe variants and reject categories outside BR taxonomy."""
    if category is None:
        return "safe"
...
def normalize_verdict_fields(violation: object, category: object) -> tuple[bool | None, str, str | None]:
    """Keep juror category output inside the BR taxonomy."""
    raw_category = category
    normalized_category = normalize_category(raw_category)
    if normalized_category is None:
        return None, "safe", f"Invalid category from model: {raw_category}"

    if normalized_category == "safe":
...
def resolve_majority_category(categories: list[str]) -> str:
    ...
    best_prio = -1
    for c in categories:
        prio = BR_CATEGORY_PRIORITY.get(c, 0)
        if prio > best_prio:
            best_prio = prio
            best = c
    return best
    ...
    for attempt in range(3):
        try:
            ...
            return {
                ...
                "violation": violation,
                "category": category,
                "confirmed_label": data.get("confirmed_label", category),
                "confidence": float(data.get("confidence", 0.0)),
                "reasoning": data.get("reasoning", ""),
                "usable": data.get("usable", None),
                "label_match": data.get("label_match", None),
...
def load_br_data(
    ...
    """Load BR annotation candidates from CSV or parquet."""
    ...
    print(f"Loaded BR data: {len(df):,} rows from {input_path}")
    ...
    if "content_id" not in df.columns:
        df["content_id"] = [f"br_train_{i:08d}" for i in range(len(df))]
...
def build_br_jury_prompts(
    ...
    """Build three juror prompts from the shared BR policy prompt."""
    policy = load_annotation_policy()
    prompts = {}
    for key in ["A", "B", "C"]:
        cfg = jury_config[key]
        system = f"""You are Juror {key}: {cfg.get("role", "Brazil content safety annotator")}.
...
def process_one_post(
    ...
    def _str(v): return str(v) if (v is not None and v == v) else ""
    title = _str(row.get("title")) or ""
    body = _str(row.get("body")) or ""
    clean_text = _str(row.get("clean_text")) or body or title
    language = _str(row.get("language")) or "pt"
    source = _str(row.get("source")) or "reddit"
    subreddit = _str(row.get("subreddit")) or ""

    # Build prompts
    prompts = build_br_jury_prompts(
        content=clean_text,
        title=title,
        source=source,
        language=language,
...
def run_batch_annotation(
    ...
    for idx, (_, row) in enumerate(to_process.iterrows()):
        ...
        _t = row.get("title"); _c = row.get("clean_text")
        title_preview = (str(_t) if (_t is not None and _t == _t and str(_t).strip()) else str(_c) if (_c is not None and _c == _c) else "")[:80]
...
def main():
    parser = argparse.ArgumentParser(
        description="Annotate Brazil data with Nvidia NIM jury",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python run_br_annotation.py                          # full train_BR.csv to br_annotation_full.csv
  python run_br_annotation.py --limit 500              # 500 posts
  python run_br_annotation.py --resume                 # resume from br_annotation_full.csv
  python run_br_annotation.py --jury-config fast       # use fast model config
  python run_br_annotation.py --input path/to/file.csv # use a custom CSV/parquet input
  python run_br_annotation.py --dry-run                # preview data only
  python run_br_annotation.py --languages pt,en        # Portuguese + English
        """,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--languages", type=str, default="pt,en",
        help="Comma-separated language codes to include (default: pt,en)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Custom output file name (saved in data/annotations_br/)"
    )
    parser.add_argument(
    ...
    if args.output:
        ...
        checkpoint_name = "br_annotation_full.csv"
    ...
    if args.resume:
        ...
        if not already_done:
            existing_checkpoints = sorted(
                list(RESULTS_DIR.glob("br_annotation_*.parquet"))
                + list(RESULTS_DIR.glob("br_annotation_*.csv")),
