#!/usr/bin/env python3
"""
Batch annotation for Saudi Arabia Arabic data using Nvidia NIM jury.
...
Usage:
  python run_sa_annotation.py                    # default: 100 posts, 3 models
  python run_sa_annotation.py --limit 500        # annotate 500 posts
  python run_sa_annotation.py --limit 1000 --resume  # resume from checkpoint
  python run_sa_annotation.py --dry-run           # show what would be annotated
"""
...
DEFAULT_INPUT = PROJECT_ROOT / "supplement_from_local_rawlabel_only" / "SA_local_candidates_for_llm_rawlabel_only.csv"
PROMPT_PATH = PROJECT_ROOT / "sa_jury_prompt.txt"
RESULTS_DIR = DATA_DIR / "annotations_sa"
...
# Top-performing models for Arabic/low-resource languages from NIM tests.
...
JURY_MODELS = {
    "A": {
        ...
        "description": "Gemma: reliable Arabic/KSA safety baseline",
        "timeout": 60,
    },
    "B": {
    ...
    "C": {
        ...
        "role": "Senior Saudi Compliance Arbitrator",
        "description": "Qwen: strong Arabic support and nuanced reasoning",
        "timeout": 120,
    },
}
...
ALTERNATIVE_JURY = {
    "fast": {
        "A": {"name": "google/gemma-3n-e4b-it", "role": "Arabic Language Specialist", "timeout": 30},
        "B": {"name": "meta/llama-3.1-8b-instruct", "role": "Cross-Cultural Reasoner", "timeout": 30},
        "C": {"name": "google/gemma-3n-e2b-it", "role": "Senior Saudi Compliance Arbitrator", "timeout": 30},
    },
    "arabic": {
        "A": {
            "name": "google/gemma-3n-e4b-it",
    ...
    "balanced": {
        "A": {"name": "meta/llama-3.1-8b-instruct", "role": "Arabic Language Specialist", "timeout": 45},
        "B": {"name": "qwen/qwen3-next-80b-a3b-instruct", "role": "Cross-Cultural Reasoner", "timeout": 45},
        "C": {"name": "mistralai/mistral-nemotron", "role": "Senior Saudi Compliance Arbitrator", "timeout": 45},
    },
    "thorough": {
        "A": {"name": "google/gemma-4-31b-it", "role": "Arabic Language Specialist", "timeout": 90},
        "B": {"name": "qwen/qwen3-coder-480b-a35b-instruct", "role": "Cross-Cultural Reasoner", "timeout": 90},
        "C": {"name": "mistralai/mistral-large-3-675b-instruct-2512", "role": "Senior Saudi Compliance Arbitrator", "timeout": 90},
    },
}

# ── Vote resolution ──────────────────────────────────────────────

# SA-specific violation categories with priority (higher = more severe)
SA_CATEGORY_PRIORITY = {
    "SA_State_Security_Royalty": 100,
    "SA_Religious_Violation": 90,
    "SA_LGBTQ_Content": 80,
    "Dangerous_Content": 70,
    "Hate_Speech": 60,
    "Harassment": 50,
    "Sexually_Explicit_Information": 40,
    "Politically_Sensitive_Topics": 30,
    "Cybersecurity_Malware": 20,
    "safe": 0,
    "none": 0,
}
...
def resolve_majority_category(categories: list[str]) -> str:
    ...
    best_prio = -1
    for c in categories:
        prio = SA_CATEGORY_PRIORITY.get(c, 0)
        if prio > best_prio:
            best_prio = prio
            best = c
    return best
...
def load_sa_data(
    ...
    """Load SA annotation candidates from CSV or parquet."""
    ...
    print(f"Loaded SA data: {len(df):,} rows from {input_path}")
    ...
    if "content_id" not in df.columns:
        df["content_id"] = [f"sa_local_{i:08d}" for i in range(len(df))]
...
def build_sa_jury_prompts(
    ...
    """Build three juror prompts from the shared V3.5 policy prompt."""
    policy = load_annotation_policy()
    prompts = {}
    for key in ["A", "B", "C"]:
        cfg = jury_config[key]
        system = f"""You are Juror {key}: {cfg.get("role", "Saudi Arabia content safety annotator")}.
...
def process_one_post(
    ...
    clean_text = str(row.get("clean_text", "") or body or title)
    language = row.get("language", "ar")
    source = row.get("source", "reddit")
    subreddit = row.get("subreddit", "saudiarabia")

    # Build prompts
    prompts = build_sa_jury_prompts(
        content=clean_text,
        title=title,
        source=source,
        language=language,
...
def run_batch_annotation(
    ...
    for idx, (_, row) in enumerate(to_process.iterrows()):
        ...
        title_preview = (row.get("title") or row.get("clean_text") or "")[:80]
...
def main():
    parser = argparse.ArgumentParser(
        description="Annotate Saudi Arabia Arabic data with Nvidia NIM jury",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python run_sa_annotation.py                          # full input to sa_annotation_full.csv
  python run_sa_annotation.py --limit 500              # 500 posts
  python run_sa_annotation.py --limit 2000 --resume    # resume from checkpoint
  python run_sa_annotation.py --jury-config fast       # use fast model config
  python run_sa_annotation.py --input path/to/file.csv # use a custom CSV/parquet input
  python run_sa_annotation.py --dry-run                # preview data only
  python run_sa_annotation.py --languages ar,arz,en    # Arabic + English
  python run_sa_annotation.py --resume --limit 0 --require-complete-jury --juror-retries 4
        """,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--jury-config", choices=["fast", "balanced", "thorough", "arabic"],
        default="arabic",
        help="Jury model configuration preset (default: arabic — optimized for Arabic)"
    )
    parser.add_argument(
        "--languages", type=str, default="ar,arz",
        help="Comma-separated language codes to include (default: ar,arz)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Custom output file name (saved in data/annotations_sa/)"
    )
    parser.add_argument(
    ...
    if args.output:
        ...
        if not checkpoint_name.endswith((".parquet", ".csv")):
            checkpoint_name += ".parquet"
    else:
        config_name = args.jury_config
        checkpoint_name = f"sa_annotation_{config_name}_{timestamp}.parquet"
    ...
    if args.resume:
        ...
        if not already_done:
            existing_checkpoints = sorted(
                list(RESULTS_DIR.glob("sa_annotation_*.parquet"))
                + list(RESULTS_DIR.glob("sa_annotation_*.csv")),
